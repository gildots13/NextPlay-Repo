...
from .models import Titles, Comment, Rating, Favorite, SearchHistory
from django.contrib.auth.decorators import login_required
...
def app(request):
    ...
    favoritos_recentes = Favorite.objects.filter(
        user=request.user
    ).select_related("title").order_by("-created_at")[:5]

    avaliacoes_recentes = Rating.objects.filter(
        user=request.user
    ).select_related("title").order_by("-updated_at", "-id")[:6]

    pesquisas_recentes = SearchHistory.objects.filter(
        user=request.user
    ).order_by("-created_at")[:8]

    total_favoritos = Favorite.objects.filter(
        user=request.user
    ).count()

    total_avaliacoes = Rating.objects.filter(
        user=request.user
    ).count()

    total_pesquisas = SearchHistory.objects.filter(
        user=request.user
    ).count()
...
def pesquisar_titulos(request):
    ...
    ultima_pesquisa = SearchHistory.objects.filter(
        user=request.user
    ).order_by("-created_at").first()

    if not ultima_pesquisa or ultima_pesquisa.term.lower() != termo.lower():
...
def salvar_avaliacao(request, title_id):
    ...
    rating_existente = Rating.objects.filter(
        user=request.user,
        title=titulo
    ).order_by("id").first()

    if rating_existente:
        rating_existente.stars = stars
...
def alternar_favorito(request, title_id):
    ...
    favoritos = Favorite.objects.filter(
        user=request.user,
        title=titulo
    )
